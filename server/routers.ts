...
export const appRouter = router({
  ...
  bookings: router({
    ...
    updateByStudent: protectedProcedure.input(z.object({ bookingId: z.string().uuid(), status: z.literal("CANCELLED") })).mutation(async ({ ctx, input }) => { requireStudent(ctx.user.appRole); const result = await db.transitionBookingStatus(ctx.supabase, input.bookingId, input.status.toLowerCase()); await sendBookingNotificationToOwner("cancelled", input.bookingId, ctx.env); return result; }),
    updateByOwner: protectedProcedure.input(z.object({ bookingId: z.string().uuid(), status: z.enum(["CONTACTED", "OWNER_CONFIRMED", "REJECTED", "CANCELLED"]) })).mutation(async ({ ctx, input }) => {
      ...
      const kindMap: Record<string, BookingNotificationKind> = { CONTACTED: "contacted", OWNER_CONFIRMED: "owner_confirmed", REJECTED: "rejected", CANCELLED: "cancelled" };
      const kind = kindMap[input.status];
      if (kind) await sendBookingNotificationToStudent(kind, input.bookingId, ctx.env);
      return result;
    }),
    updateByStaff: adminProcedure.input(z.object({ bookingId: z.string().uuid(), status: z.literal("COMPLETED") })).mutation(async ({ ctx, input }) => { const result = await db.transitionBookingStatus(ctx.supabase, input.bookingId, input.status.toLowerCase() as z.infer<typeof bookingStatus>); await sendBookingNotificationToStudent("completed", input.bookingId, ctx.env); await sendBookingNotificationToOwner("completed", input.bookingId, ctx.env); return result; }),
    ...
    scheduleViewing: adminProcedure.input(z.object({ bookingId: z.string().uuid(), viewingAt: z.string().datetime({ offset: true }) })).mutation(async ({ ctx, input }) => { const result = await db.scheduleViewing(ctx.supabase, input.bookingId, input.viewingAt); await sendBookingNotificationToStudent("rescheduled", input.bookingId, ctx.env); await sendBookingNotificationToOwner("rescheduled", input.bookingId, ctx.env); return result; }),
    completeViewing: adminProcedure.input(z.object({ bookingId: z.string().uuid() })).mutation(async ({ ctx, input }) => { const result = await db.completeViewing(ctx.supabase, input.bookingId); await sendBookingNotificationToStudent("completed", input.bookingId, ctx.env); await sendBookingNotificationToOwner("completed", input.bookingId, ctx.env); return result; }),
    recordStudentDecision: protectedProcedure.input(z.object({ bookingId: z.string().uuid(), decision: z.enum(["ACCEPTED", "REJECTED"]) })).mutation(async ({ ctx, input }) => { requireStudent(ctx.user.appRole); const result = await db.recordStudentViewingDecision(ctx.supabase, input.bookingId, input.decision.toLowerCase() as "accepted" | "rejected"); await sendBookingNotificationToOwner("student_confirmed", input.bookingId, ctx.env); return result; }),
    ...
    staffReschedule: adminProcedure.input(z.object({ bookingId: z.string().uuid(), viewingAt: z.string().datetime({ offset: true }), reason: appointmentExceptionReason })).mutation(async ({ ctx, input }) => { const result = await db.staffRescheduleViewing(ctx.supabase, input.bookingId, input.viewingAt, input.reason); await sendBookingNotificationToStudent("rescheduled", input.bookingId, ctx.env); await sendBookingNotificationToOwner("rescheduled", input.bookingId, ctx.env); return result; }),
    staffCancel: adminProcedure.input(z.object({ bookingId: z.string().uuid(), reason: appointmentExceptionReason })).mutation(async ({ ctx, input }) => { const result = await db.staffCancelViewing(ctx.supabase, input.bookingId, input.reason); await sendBookingNotificationToStudent("cancelled", input.bookingId, ctx.env); await sendBookingNotificationToOwner("cancelled", input.bookingId, ctx.env); return result; }),
  ...
  admin: router({
    ...
    review: adminProcedure.input(z.object({ propertyId: z.string().uuid(), status: z.enum(["VERIFIED", "NEEDS_CHANGES", "REJECTED"]), reason: z.string().max(1000).optional(), mediaQualityIssue: z.boolean().optional(), ownerIdentityVerified: z.boolean().optional(), propertyVideoVerified: z.boolean().optional(), locationVerified: z.boolean().optional(), availabilityVerified: z.boolean().optional(), propertyPhotosQualityVerified: z.boolean().optional() })).mutation(async ({ ctx, input }) => { if (input.status !== "VERIFIED" && !input.reason?.trim()) throw new Error("سبب القرار مطلوب عند طلب تعديل أو رفض الإعلان."); if (input.status === "REJECTED" && input.mediaQualityIssue && input.reason!.trim().length < 12) throw new Error("اكتب سبباً واضحاً لا يقل عن 12 حرفاً عند رفض الصور لجودتها أو ملاءمتها."); const result = await db.reviewProperty(ctx.supabase, ctx.user.id, { ...input, status: input.status.toLowerCase() }); const decision = input.status.toLowerCase() as "verified" | "needs_changes" | "rejected"; await sendPropertyReviewEmail(input.propertyId, decision, input.reason, ctx.env); return result; }),
    publishMedia: adminProcedure.input(z.object({ propertyId: z.string().uuid(), media: z.array(z.object({ mediaId: z.string().uuid(), webpBase64: z.string().min(1) })).min(1).max(db.MAX_PROPERTY_PHOTO_COUNT) })).mutation(({ ctx, input }) => db.publishApprovedMediaFromBytes(input.propertyId, input.media)),
  }),
  media: router({
  ...
  leads: router({
    ...
    })).mutation(async ({ input, ctx }) => { const result = await db.submitOwnerLead(input); await sendOwnerLeadEmail(result.leadId, input.name, input.phone, input.area, ctx.env); return result; }),
